...
class BotCore(commands.Bot):
    ...
    async def on_message(self, message: disnake.Message):
        ...
        if message.content == f"<@{self.user.id}>" or message.content == f"<@!{self.user.id}>":
            ...
            embed = disnake.Embed(
                description=f"Olá, utilize **/help** para ver todos os meus comandos!\n"
                            f"Caso os comandos não apareçam use **/authorizebot** para mais informações!\n",
                color=self.get_color(message.guild.me)
            )

            if self.slash_commands:
                embed.description += f"Você também pode procurar meus comandos de barra caso nâo queira usar o **/help**."
